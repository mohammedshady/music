<template>
   <div class="fixed bottom-0 left-0 bg-white p-5 pb-4 text-left align-top w-full h-16">
    <div class="relative">
      <!-- Play/Pause Button -->
      <div class="float-left w-14 h-7 leading-3">
        <button type="button" @click.prevent="toggleAudio">
          <i class="fa text-gray-500 text-xl"
          :class="{ 'fa-play': !playing, 'fa-pause': playing }"></i>
        </button>

         <button type="button" @click.prevent="muteAudio">
          <i class="fas text-gray-500 text-xl ml-2"
          :class="{ 'fa-volume-off': !muted, 'fa-volume-up': muted }"></i>
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions(['toggleAudio', 'updateSeek', 'muteAudio']),
  },
